...
@patch("time.sleep")
class TestGetJobResultRows(unittest.TestCase):
    """`AdsInsights.get_job_result_rows` wraps `job.get_result()` with
    `retry_on_adreport_job_not_ready_error` (tap_facebook/retry_helpers.py) 
    to survive Facebook's async-job eventual-consistency race: the job can
    report "Job Completed" while polling, but the report data isn't queryable yet,
    producing a 400 "The adreport job is not completed yet" error on the
    immediately-following `get_result()` call.
    """
    ...
    def test_retries_on_adreport_job_not_completed_error(self, mocked_sleep):
        ...
        with self.assertRaises(FacebookRequestError):
            ad_insights_object.get_job_result_rows(mocked_job)
    ...
    def test_retries_and_succeeds_once_job_is_ready(self, mocked_sleep):
        ...
        `get_job_result_rows` should return the successful result.
        """
        ...
        ad_insights_object = AdsInsights('', Mock(), '', '', {}, {})
        result = ad_insights_object.get_job_result_rows(mocked_job)
